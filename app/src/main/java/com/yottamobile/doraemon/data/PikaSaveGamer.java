...
public class PikaSaveGamer {
	...
	public static boolean getSound(String key) {
		SharedPreferences prefs = PikachuActivity.getPikachu().getSharedPreferences(GAME, Context.MODE_PRIVATE);
	...
	public static String defaultLevel() {
		...
		try {
			...
			for (int i = 1; i < PikachuActivity.LVL_MAX; i++) {
	...
	public static void unlockAllMap() {
		JSONArray arr = new JSONArray();
		try {
			for (int i = 0; i < PikachuActivity.LVL_MAX; i++) {
				JSONObject objLv = new JSONObject();
