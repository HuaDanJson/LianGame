...
public class PikaHomeScene extends CustomScene implements IMyScene, OnClickListener {
	...
	public void goIn() {
		...
		JumpModifier jump = new JumpModifier(0.5f, Pikachu.CAMERA_WIDTH + Pikachu.getPikachu().textureFish.getWidth(), 283, 178, 178, 200) {
			@Override
			protected void onModifierFinished(IEntity pItem) {
				countEffect++;
	...
	public void goOut() {
		countEffect = 0;
		MoveYModifier moveBanner = new MoveYModifier(0.5f, banner.getY(), -Pikachu.getPikachu().background.getHeight(), EaseBackInOut.getInstance()) {
			@Override
			protected void onModifierFinished(IEntity pItem) {
				countEffect++;
				super.onModifierFinished(pItem);
		...
		MoveXModifier moveBtnSoundBG = new MoveXModifier(0.5f, btnSoundBg.getX(), -Pikachu.getTextureRegionSoundBG().getWidth() - 500,
		...
		MoveXModifier moveBtnSoundGame = new MoveXModifier(0.5f, btnSoundGame.getX(), -Pikachu.getTextureRegionSoundBG().getWidth() - 500,
				EaseBackInOut.getInstance()) {
			@Override
			protected void onModifierFinished(IEntity pItem) {
				countEffect++;
		...
		MoveXModifier moveBtnInfo = new MoveXModifier(0.5f, btnInfo.getX(), Pikachu.CAMERA_WIDTH + 500, EaseBackInOut.getInstance()) {
			@Override
			protected void onModifierFinished(IEntity pItem) {
				countEffect++;
				super.onModifierFinished(pItem);
		...
		MoveXModifier moveBtnQuestion = new MoveXModifier(0.5f, btnQuestion.getX(), Pikachu.CAMERA_WIDTH + 500, EaseBackInOut.getInstance()) {
			@Override
			protected void onModifierFinished(IEntity pItem) {
				countEffect++;
				super.onModifierFinished(pItem);
		...
		JumpModifier jump = new JumpModifier(0.5f, btnFish.getX(), -Pikachu.getPikachu().textureFish.getWidth() - 20, 178, 178, 200) {
			@Override
			protected void onModifierFinished(IEntity pItem) {
				countEffect++;
				super.onModifierFinished(pItem);
	...
	public Scene onCreateScene(BaseGameActivity game, Camera mCamera) {
		...
		final Sprite bg = new Sprite(0, 0, Pikachu.getPikachu().background, game.getVertexBufferObjectManager());
		bg.setVisible(false);
		attachChild(bg);

		banner = new Sprite(156, Pikachu.CAMERA_HEIGHT / 2 - Pikachu.getPikachu().regionBanner.getHeight() / 2, Pikachu.getPikachu().regionBanner,
				game.getVertexBufferObjectManager());
		
		playNow = new Sprite(243, 390, Pikachu.getPikachu().regionPlaynow, game.getVertexBufferObjectManager());
		playNow.setVisible(false);
		attachChild(playNow);
		...
		DelayModifier delay = new DelayModifier(1) {
			...
			protected void onModifierFinished(IEntity pItem) {
				...
				btnFish = new MButton(Pikachu.CAMERA_WIDTH + Pikachu.getPikachu().textureFish.getWidth(), 178, Pikachu.getPikachu().textureFish,
				...
				btnSoundBg = new MButton(-Pikachu.getTextureRegionSoundBG().getWidth() - 500, 315, Pikachu.getTextureRegionSoundBG(),
						PikaHomeScene.this.game.getVertexBufferObjectManager());
				MoveXModifier moveSoundBg = new MoveXModifier(0.8f, btnSoundBg.getX(), 15, EaseBackInOut.getInstance());
				btnSoundBg.setUserData(Pikachu.BTN_SOUND_BG);
				...
				btnSoundGame = new MButton(-Pikachu.getTextureRegionSoundBG().getWidth() - 500, 391, Pikachu.getTextureRegionSoundGame(),
						PikaHomeScene.this.game.getVertexBufferObjectManager());
				MoveXModifier moveSoundGame = new MoveXModifier(0.8f, btnSoundGame.getX(), 15, EaseBackInOut.getInstance());
				btnSoundGame.setUserData(Pikachu.BTN_SOUND_GAME);
				...
				btnInfo = new MButton(Pikachu.CAMERA_WIDTH + 500, 315, Pikachu.getPikachu().regionIconInfo,
						PikaHomeScene.this.game.getVertexBufferObjectManager());
				...
				btnQuestion = new MButton(Pikachu.CAMERA_WIDTH + 500, 391, Pikachu.getPikachu().regionIconQuestion,
						PikaHomeScene.this.game.getVertexBufferObjectManager());
	...
	public void onClick(ButtonSprite pButtonSprite, float pTouchAreaLocalX, float pTouchAreaLocalY) {
		...
		} else if (pButtonSprite.getUserData().equals(Pikachu.BTN_SOUND_BG)) {
			Pikachu.setSoundBG(!Pikachu.isSoundBG());

			btnSoundBg.attachChild(new MButton(0, 0, Pikachu.getTextureRegionSoundBG(), PikaHomeScene.this.game.getVertexBufferObjectManager()));

		} else if (pButtonSprite.getUserData().equals(Pikachu.BTN_SOUND_GAME)) {
			Pikachu.setSoundGame(!Pikachu.isSoundGame());
			btnSoundGame.attachChild(new MButton(0, 0, Pikachu.getTextureRegionSoundGame(), PikaHomeScene.this.game.getVertexBufferObjectManager()));
		} else if (pButtonSprite.getUserData().equals(BTN_QUESTION)) {
			// unlock map
//			PikaSaveGamer.unlockAllMap();
			Pikachu.getPikachu().vData = PikaSaveGamer.getDataLevel();
			showTutorial();
...
//			PikaSaveGamer.lockAllMap();
			...
			Sprite bg = new Sprite(0, 0, Pikachu.getPikachu().regionBangDiem, PikaHomeScene.this.game.getVertexBufferObjectManager());
			pop.registerEntityModifier(move);
			pop.attachChild(bg);


			
			Text mMission = new TickerText(100, -70, Pikachu.getPikachu().mStrokeFont,
			...
			MButton btnRetry = new MButton(380, 358, Pikachu.getPikachu().textureBack, game.getVertexBufferObjectManager()) {
				@Override
